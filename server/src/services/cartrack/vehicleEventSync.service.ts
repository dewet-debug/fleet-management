...
export async function syncVehicleEvents(client: CartrackApiClient, sinceDate?: Date): Promise<SyncResult> {
  ...
  let events: any[];
  try {
    events = (await client.getVehicleEvents({
    ...
    if (skip) {
      console.warn(`[Cartrack] Vehicle-events sync skipped (${skip} — not permitted or outside allowed window).`);
      return result;
    }
    console.error('[Cartrack] Error fetching vehicle events:', err);
    result.errored++;
    return result;
  }
  ...
  for (let idx = 0; idx < events.length; idx++) {
    ...
    const eventId = String(
      e.event_id ?? e.id ?? e.vehicle_event_id ?? `${fleetVehicleId}-${e.event_time ?? e.timestamp ?? idx}`
    );
    ...
    try {
      ...
      const existing = await prisma.cartrackVehicleEvent.findUnique({
        where: { cartrackEventId: eventId },
      });

      if (existing) {
        await prisma.cartrackVehicleEvent.update({ where: { cartrackEventId: eventId }, data: eventData });
        result.updated++;
      } else {
        await prisma.cartrackVehicleEvent.create({ data: { cartrackEventId: eventId, ...eventData } });
        result.created++;
      }
    } catch (err) {
      console.error(`[Cartrack] Error upserting vehicle event ${eventId}:`, err);
      result.errored++;
    }
  }
