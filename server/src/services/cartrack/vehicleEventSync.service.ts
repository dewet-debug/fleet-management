...
export async function syncVehicleEvents(client: CartrackApiClient, sinceDate?: Date): Promise<SyncResult> {
  ...
  for (const fv of fleetVehicles) {
    try {
      const events = (await client.getVehicleEvents({
      ...
      for (let idx = 0; idx < events.length; idx++) {
        ...
        const eventId = String(
          e.event_id ?? e.id ?? e.vehicle_event_id ?? `${fv.id}-${e.event_time ?? e.timestamp ?? idx}`
        );
        ...
        try {
          ...
          if (existing) {
            await prisma.cartrackVehicleEvent.update({
              where: { cartrackEventId: eventId },
              data: eventData,
            });
            result.updated++;
          } else {
            await prisma.cartrackVehicleEvent.create({
      ...
      console.error(`[Cartrack] Error fetching vehicle events for vehicle ${fv.licensePlate}:`, err);
      result.errored++;
    }
  }
